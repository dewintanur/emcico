...
@section('content')
    <script>
        ...
        // Jalankan update setiap 5 detiki
        setInterval(updateTable, 5000);
        setInterval(checkNotification, 5000);

        function checkNotification() {
            fetch('/notifications/${notificationId}/read`') // Ganti dengan endpoint notifikasi di backend kamu
                .then(response => response.json())
                .then(data => {
                    if (data.ada_notifikasi) {
                        alert('Anda punya notifikasi baru!');
                        // Bisa juga update elemen notifikasi tanpa alert
                    }
                })
        ...
        <div class="row align-items-center mb-3" style="margin-top: -10px; ">
            <div class="col-md-3">
                <form method="GET" action="{{ route('fo.bookingList') }}">
                    <select name="status" class="form-select" style="width: 100%; border-radius:10px;" aria-label="Status Filter"
                                    ...
                                    <tr style="width:100%;">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td style="width: 10%;">
                                                            ...
                                                            @else
                                                                ...
                                                                <a href="{{ route('checkin', ['kode_booking' => $booking->kode_booking]) }}"
                                       ...
                                       <div class="modal fade" id="dutyOfficerModal{{ $booking->id }}" tabindex="-1"
                                                ...
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title" id="modalLabel{{ $booking->id }}">Pilih Duty Officer
                                                            </h5>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                                aria-label="Close"></button>
                                                        </div>
